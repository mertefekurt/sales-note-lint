...
from sales_note_lint import __version__
from sales_note_lint.core import (
    audit_records,
    read_records,
    render_json,
    render_markdown,
...
from sales_note_lint.rules import PROJECT_NAME


def build_parser() -> argparse.ArgumentParser:
    parser = argparse.ArgumentParser(
        prog='sales-note-lint',
        description=(
                        'Check sales call notes for missing next step, decision maker, and timeli'
                        'ne.'
                    ),
    )
    parser.add_argument("--version", action="version", version=f"%(prog)s {__version__}")
    parser.add_argument("input", nargs="?", type=Path, help='sales call note')
    parser.add_argument(
        "--format",
...
def main(argv: list[str] | None = None) -> int:
    ...
    try:
        ...
        print(f"sales-note-lint: error: {exc}", file=sys.stderr)
        return 1
